...
export class GitHubAdapter implements RepositoryAdapter {
   ...
   * @returns 包含文件信息的对象
   */
  private async fetchFile(owner: string, repo: string, path: string, branch?: string): Promise<{ path: string; type: 'file'; content: string }> {
    ...
    return {
      path: path,
      type: 'file',
      content: new TextDecoder().decode(Uint8Array.from(atob(response.data.content || ''), c => c.charCodeAt(0)))
    };
  }
   ...
   */
  private async fetchDirectory(owner: string, repo: string, path: string, branch?: string): Promise<Array<{ path: string; type: 'file' | 'directory'; content: string }>> {
    const response = await this.octokit.repos.getContent({
      owner,
      repo,
      path,
   ...
   * @returns 包含所有文件信息的对象
   */
  private convertToWebContainerFormat(files: Array<{ path: string; type: 'file' | 'directory'; content: string }>) {
