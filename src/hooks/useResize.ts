...
import { bottomPanelAtom, bottomPanelOpenAtom, leftPanelAtom, leftPanelOpenAtom, resolveBottomPanelAtom, resolveLeftPanelAtom } from "@/store/global"
...
export function useResize() {
  ...
  // 左侧面板的拖拽事件
  const { run: leftPanelResize } = useThrottleFn((size: number) => {
    if (!size) {
      // 宽度为0，设置不可见
  ...
  // 底部面板的拖拽事件
  const { run: bottomPanelResize } = useThrottleFn((size: number) => {
    if (!size) {
      // 高度为0，设置不可见
